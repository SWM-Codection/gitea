...
export default {
  ...
  data: () => ({
    store: repositoryFileStore(),
    collapsed: false,
  }),
  methods: {},
  mounted() {
    console.log("selected item =>", this.store.selectedItem)
  },
};
</script>
...
<template>
  ...
  <div v-if="item.children?.length" v-show="!collapsed" class="sub-items">
    <DiscussionFileTreeItem v-for="childItem in item.children" :key="childItem.name" :item="childItem"/>
  </div>
</template>
