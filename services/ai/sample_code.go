...
int main() {
	...
	// 3개의 동일한 코드를 각 response에 담음
	for i := 0; i < AI_SAMPLE_CODE_UNIT; i++ {
		go func() {
			defer wg.Done()

			// 하이라이트 적용 (예시)
			highlightedCode, _ := highlight.Code(comment.TreePath, "cpp", codeContent)


			resultQueue <- &AiSampleCodeResponse{
				SampleCode: string(highlightedCode),
			}
		}()
	}
